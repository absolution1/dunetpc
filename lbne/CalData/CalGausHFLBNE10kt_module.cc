...
namespace calgaushf {
  ...
  {      
	...
	//sss->SignalShaping(channel).CalculateDeconvKernel();
	//std::vector<TComplex>  kerndec = sss->SignalShaping(channel).DeconvKernel();
	
	//std::vector<TComplex>  kerndec_truncated(fDeconvKernSize,0);
	//Truncate deconvolution kernel
	//kerndec.resize(fDeconvKernSize);
...
// 	  std::vector<double> kernrc(kerndec.size());
// 	  for(unsigned int i=0; i<kerndec.size(); ++i)
// 	    kernrc[i] = kerndec[i].Rho();

// 	  sprintf(histogramname,"kern_deconv_c_%lu",rdIter); 
// 	  TH1D* hkernc = dirc.make<TH1D>(histogramname, "Collection Deconvolution Kernel", fDeconvKernSize+1, -0.5, fDeconvKernSize+0.5);
// 	  hkernc->SetMinimum(0.);
// 	  vector_to_hist(kernrc, hkernc);

	  
// 	  //sprintf(filename,"/lbne/data/users/jti3/kernels/coll_%lu.root",rdIter);
// 	  //TFile *f = new TFile(filename,"recreate");
// 	  //hkernc->Write();
// 	  //f->Close();
// 	}
// 	else if(sigType==geo::kInduction){
// 	  //if(inductioncount>0) continue;
// 	    inductioncount++;
// 	    std::vector<double> kernri(kerndec.size());
// 	    for(unsigned int i=0; i<kerndec.size(); ++i)
// 	      kernri[i] = kerndec[i].Rho();
// 	    sprintf(histogramname,"kern_deconv_i_%lu",rdIter); 
// 	    TH1D* hkerni = dirc.make<TH1D>(histogramname, "Induction Deconvolution Kernel", fDeconvKernSize+1, -0.5, fDeconvKernSize+0.5);
// 	    hkerni->SetMinimum(0.);
// 	    vector_to_hist(kernri, hkerni);
// 	    //TFile f("/lbne/data/users/jti3/induction_deconvolution_kernel.root");
// 	    //sprintf(filename,"/lbne/data/users/jti3/kernels/ind_%d.root",inductioncount);
// 	    //TFile *f = new TFile(filename,"recreate");
// 	    //hkerni->Write();
// 	    //f->Close();
// 	}	
...
// 	  } 
	  ...
	  //if(wirecol->size() == 0)
	  //mf::LogWarning("CalGausHFLBNE10kt") << "No wires made for this event.";
