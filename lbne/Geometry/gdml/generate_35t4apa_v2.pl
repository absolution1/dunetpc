...
$APA_z = 51.99; # DocDb 7550: "overall width"
                # 144 U/V each, good

$APAActive_z = 49.95; # DocDb 7550: "active width"

 # NUMBER VERTICAL WIRES = (APAActive_z / pitch) + 1
   #   Since APAActive_z is defined in docdb 7550 to be distance 
...
$OriginYSet             =       $DetEnc_y/2 
                              ...
                              - $SteelThickness
                              - $SpaceAPAToFloor
                              - $APAHeight[2] #using tpc heights would include gap, we dont want to
                              - $APAVerticalGap/2;

$OriginYSetCheck        =     - $DetEnc_y/2
                              + $TotalPadding
                              + $SteelThickness
                              + $HeightGaseousAr
                              + $SpaceAPAToTopLAr
...
$posTPCl_z_upstream    =    - $Argon_z/2
                            + $UpstreamLArPadding ## this already steps into the APALongGap/2 space on outside of TPC...
                            + $APA_z/2;
...
#TODO: maybe define APA centers more fundamentally and then place TPC centers based off of this
...
# $_[3] = 'name'
    ...
    my $TPCWirePlaneHeight = $_[1] - $APAVerticalGap;
    my $TPCWirePlaneLength = $_[2] - $APALongGap;
...
$NumberCommonVWires = $NumberSideVWires - $NumberCornerVWires;
   ...
   #   between outer vertical wires, + 1 since the floor of this
   #   division will be one under, giveing the amt of spaces, not wires
$NumberVerticalWires = int( $APAActive_z/$XWirePitch + 1 );
}
...
{
my $zpos = (-0.5*$APAActive_z)+$XWirePitch*($i+0.5)+$TPCWireThickness/2;
