...
private:
  ...
  std::vector<double> CRT_TOP_center={-581,305.7,135};//drift in Y geometry in cm
  std::vector<double> CRT_BOT_center={+581,-202.7,135}; //drift in Y geometry in cm
  double CRTHeightY=8*14.0; //size in Y
...
//  int 	 	 firstMother    = 0;
  ...
  double 	 xPosition   	= CRT_TOP_center[0];
  ...
  double 	 time        	= 0.;
  double 	 xPositionEnd  	= CRT_BOT_center[0];
  ...
  std::cout << "Shooting muon on " << xPosition << " " << yPosition << " " << zPosition << " With momentum: " << xMomentum << " " << yMomentum << " " << zMomentum << " E=" << energy << " m=" << mass << std::endl;
