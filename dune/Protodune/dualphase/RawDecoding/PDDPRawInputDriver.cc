...
namespace lris
{
				     ...
				     art::FileBlock* &fb )
  {
    ...
    //
    __file.open( name.c_str(), ios::in | ios::binary | ios::ate);
				      ...
				      art::EventPrincipal* &outE )
  {
    ...
    if( __events[ __eventCtr ] != __file.tellg() )
      __file.seekg( __events[ __eventCtr ], ios::beg );
    ...
    DaqEvent event;
    bool ok = __unpackEvent( buf, event );
    ...
    if( doupdate = (srn != __currentSubRunId.subrun()) )
      {
	outSR = __sourceHelper_.makeSubRunPrincipal(rn, srn, tstamp);
      }
  ...
  unsigned PDDPRawInputDriver::__unpack_evtable()
  {
    ...
    mf::LogDebug(__FUNCTION__)<<"Number of events in this file "<<__eventNum<<"\n";
    ...
    //for( auto e : __events ) mf::LogDebug(__FUNCTION__)<<e<<"\n";
    //for( auto e : __evsz ) mf::LogDebug(__FUNCTION__)<<e<<"\n";
