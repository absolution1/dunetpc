...
namespace CRT
{
  class Trigger
  {
    public:
      ...
      Trigger(): fChannel(-1), /*fDetName(""),*/ fTimestamp(-1), fHits() {} //Default constructor to satisfy ROOT.  
      ...
      unsigned long long Timestamp() const { return fTimestamp; }
      const std::vector<CRT::Hit>& Hits() const { return fHits; }

      //Check whether this Hit was default-constructed
      bool IsDefault() const { return fChannel == -1; }

      //Dumping an object is also helpful for debugging.  
      template <class STREAM>
      STREAM& dump(STREAM& stream) const
      {
  ...
  class Hit
  {
    public:
      ...
      //hits are obvious.  
      Hit(): fChannel(-1), /*fAuxDetName("NULL"),*/ fADC(-1) {}

      //No resources managed by a CRT::Hit, so use default destructor
      virtual ~Hit() = default; 
  
      //Public access to stored information
      size_t Channel() const inline { return fChannel; }

      //std::string AuxDetName() const inline { return fAuxDetName; }

      short ADC() const inline { return fADC; }

      //Check whether this Hit was default-constructed
      bool IsDefault() const { return fADC == -1; }
      ...
      STREAM& dump(STREAM& stream) const
      {
        stream << "CRT::Hit Dump:\n"
               ...
               << "ADC: " << fADC << "\n"
               << "Was this CRT::Hit default-constructed? " << IsDefault()?"yes":"no" << "\n";
        return stream;
      }
