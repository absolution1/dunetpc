...
{
    print "\t\tCreating Iceberg geometry.\n";
}
...
$APAFrame_z   = 117. - 2*(2*$G10thickness+$WrapCover);
...
# and open it
    $DEF = "iceberg_v1_Def" . $suffix . ".gdml";
    push (@gdmlFiles, $DEF);
    $DEF = ">" . $DEF;
...
# and open it
    $MAT = "iceberg_v1_Materials" . $suffix . ".gdml";
    push (@gdmlFiles, $MAT);
    $MAT = ">" . $MAT;
...
# add file to list of output GDML fragments,
# and open it
    $TPC = "iceberg_v1_TPC_${_[3]}" . $suffix . ".gdml";
    push (@gdmlFiles, $TPC);
    $TPC = ">" . $TPC;
...
# add file to list of output GDML fragments,
# and open it
    $CRYO = "iceberg_v1_Cryostat" . $suffix . ".gdml";
    push (@gdmlFiles, $CRYO);
    $CRYO = ">" . $CRYO;
...
EOF
        ...
        elsif( $workspace == 1 || $workspace==2 || $workspace==3 ){
...
# add file to list of output GDML fragments,
# and open it
    $ENCL = "iceberg_v1_DetEnclosure" . $suffix . ".gdml";
    push (@gdmlFiles, $ENCL);
    $ENCL = ">" . $ENCL;
...
# add file to list of output GDML fragments,
# and open it
    $WORLD = "iceberg_v1_World" . $suffix . ".gdml";
