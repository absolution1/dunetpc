  ...
  m_state(new State)
{
  ...
  if ( m_plotPhase ) {
    ...
    if ( !m_varPhase && !m_varEvent && !m_varTick0 ) {
...
int AdcTickModViewer::makePhaseGraphs() const {
  ...
  for ( IndexVectorMap::value_type iich : state().phaseChannels ) {
    ...
    // Build the graph.
    string hnam = "hphtm";
    string vname = m_varPhase ? "Phase" : m_varEvent ? "Event" : m_varTick0 ? "Tick0" : "UNKNOWN";
