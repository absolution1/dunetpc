...
DataMap AdcDataPlotter::viewMap(const AdcChannelDataMap& acds) const {
  ...
  for ( IndexRange ran : m_crs ) {
    ...
    // Create histogram.
    TH2* ph = new TH2F(hname.c_str(), htitl.c_str(), ntick, tick1, tick2, nchan, chanFirst, chanLast);
