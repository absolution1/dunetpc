...
int test_AdcRoiViewer(bool useExistingFcl =false) {
  ...
  if ( ! useExistingFcl ) {
    ...
    fout << "  {name:\"hcsHeight_%CRNAME%\" title:\"Pulse heights for %LAB0% %LAB1% run %RUN% %CRLABEL%\" "
         <<    "valHist:\"hfh_%0RUN%_chan%0CHAN%\" valType:fitMean errType:fitSigma bins:0 cr:apa1x plot:\"\" pran:\"\"}," << endl;
    fout << "  {name:\"hcsWidth_%CRNAME%\" title:\"Shaping times for run %RUN% %CRLABEL%\" "
         <<    "valHist:\"hfw_%0RUN%_chan%0CHAN%\" valType:fitMean errType:fitSigma bins:0 cr:apa1x plot:\"\" pran:\"\"}" << endl;
    fout << "]" << endl;
