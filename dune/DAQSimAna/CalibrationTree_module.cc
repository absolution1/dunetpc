...
namespace CalibrationTree {
  ...
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  CalibrationTree::CalibrationTree(fhichl::ParameterSet const& pSet)
    :priv_HitLabel(pSet.get<art::InputTag>("HitLabel","gausshit")),
    priv_OpHitLabel(pSet.get<art::InputTag>("OpHitLabel","ophit"))
  {  }

  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  CalibrationTree::CalibrationTree(fhiclcConfig const& config)
    :priv_HitLabel(config.HitLabel()),
    priv_OpHitLabel(config.OpHitLabel())
  {  }

  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  void FillJHRV(int trackId, art::Ptr<recob::Hit> hit, int hitIndex, double split){ //Factor out everything that doesn't require hit (all the trackID stuff) into it's own function (as it is duplicated in FillJHRV for ophits) or turn this into a template.
    simb::MCParticle* tIdToPart = PIS->TrackIdToParticle(trackId);
    simb::MCParticle* eve       = PIS->TrackIdToParticle(PIS->TrackIdToEveId(trackId));
    bool findPart = [](auto& a){ return a.particle==tIdToPart;}
    bool findEve  = [](auto& a){ return a.particle==eve;}
    std::vector<jointHitRecord>::iterator partRec = std::find_if(jhrV.begin(), jhrV.end(), findPart);
    std::vector<jointHitRecord>::iterator eveRec = std::find_if(jhrV.begin(), jhrV.end(), findPart);
    ...
    partRec = std::find_if(jhrV.begin(), jhrV.end(), findPart);//update partRec (in case we added the particle ourselves.
  ...
  //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  void FillJHRV(int trackId, art::Ptr<recob::OpHit> hit, double ophitIndex, double split){
    simb::MCParticle* tIdToPart = PIS->TrackIdToParticle(trackId);
    simb::MCParticle* eve       = PIS->TrackIdToParticle(PIS->TrackIdToEveId(trackId));
    bool findPart = [](auto& a){ return a.particle==tIdToPart;}
    bool findEve  = [](auto& a){ return a.particle==eve;}
  ...
  virtual void beginJob(){
    ...
    private_CalibrationCharge = private_CalibrationParticle->Branch("hits",&private_hitBuffer,"charge/d:time/d:width/d:wire/i:hitIndex/i");
    private_CalibrationLight = private_CalibrationParticle->Branch("ophits", &private_lightBuffer, "pes/d:time/d:width/d:opchan/i:opdet/i:opHitIndex/i");
  }
  ...
  virtual void analyse(const art::Event& evt) override{
    ...
    for ( auto const& hit : hitList){
      ...
      if(IDEs_Ps.empty()){
        ...
        for( auto const& ide_P : IDEs_Ps){
          ...
          this->FillJHRV(trackId, hit, hitCounter, split);
    ...
    for ( auto const& ophit : opHitList ){
      ...
      if(SDPs_Ps.empty()){
        ...
        for( auto const& sdp_P : SDPs_Ps){
          ...
          this->FillJHRV(trackId, ophit, hitCounter, split);
        }//end for sdp_P
    ...
    for ( auto jhr : jhrV){
      ...
      for(auto& htuple : jhr->hitsRec){
        ...
        private_hitBuffer.update(hit->Integral();, hit->PeakTime(), hit->Width(), (std::get<2>(htuple)/total), hit->Wire(), std::get<1>(htuple), partTracker);
      ...
      for(auto& htuple : jhr->ohitsRec){
        auto const& hit = std::get<0>(htuple);
        private_lightBuffer.update(hit->PE();, hit->PeakTime(), hit->Width(), hit->OpChannel(), (std::get<2>(htuple)/total), hit->Wire(), std::get<1>(htuple), partTracker);
