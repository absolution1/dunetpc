...
// ROOT script using gallery to make a correlation plot of raw::RawDigits for the ievcount'th event in the file, using ticks 800 to 1000
// Tom Junk, Sep. 2017
	...
	if (!rawdigits.empty())
	  {
	    ...
	    TH2D *correl = (TH2D*) new TH2D("correl","Correl",nchans,-0.5,nchans-0.5,nchans,-0.5,nchans-0.5);
		 ...
		 sumx += x;
		 for (size_t ichan=0;ichan<nchans;++ichan) sumxy[ichan] += x[ichan]*x;
		...
		for (size_t ichan2=0;ichan2<nchans;++ichan2)
		  {
		    double c = ( nticks*sumxy[ichan][ichan2] -
