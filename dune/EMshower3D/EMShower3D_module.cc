...
private:
  recob::Track ConvertFrom(const pma::Track3D& src);
	...
	//std::vector< ems::Shower2DAlg* > CollectShower2D(art::Event const & e);
	std::vector< ems::DirOfGamma* > CollectShower2D(art::Event const & e);
	//void Link(art::Event const & e, std::vector< ems::Shower2DAlg* > input);
	void Link(art::Event const & e, std::vector< ems::DirOfGamma* > input);

	//void Make3DSeg(art::Event const & e, std::vector< ems::Shower2DAlg* > pair);
  ...
  if (e.getByLabel(fCluModuleLabel, fCluListHandle))
	{
		...
		//std::vector< ems::Shower2DAlg* > showernviews = CollectShower2D(e);
...
//std::vector< ems::Shower2DAlg* > ems::EMShower3D::CollectShower2D(art::Event const & e) 
std::vector< ems::DirOfGamma* > ems::EMShower3D::CollectShower2D(art::Event const & e) 
{
	
	//std::vector< ems::Shower2DAlg* > input;
	...
	for (unsigned int c = 0; c < fCluListHandle->size(); c++)
	{
			...
			if (hits_out.size() > 5)
			{
				...
				//ems::Shower2DAlg * sh = new ems::Shower2DAlg(hits_out, 14, c);
...
//void ems::EMShower3D::Link(art::Event const & e, std::vector< ems::Shower2DAlg* > input)
void ems::EMShower3D::Link(art::Event const & e, std::vector< ems::DirOfGamma* > input)
{
	...
	while (i < input.size())	
	{
		...
		//std::vector< ems::Shower2DAlg* > pairs;
...
//void ems::EMShower3D::Make3DSeg(art::Event const & e, std::vector< ems::Shower2DAlg* > pair)
